...
function processPath(pathname) {
    // Since `pathname` can be a glob pattern, we need the try-catch around
    // `fs.statSync` calls. We cannot use `fs.existsSync` since it is deprecated.
    try {
        var fileStat = fs.statSync(pathname);

        var newPath = fileStat.isDirectory() ? path.join(pathname, "**") : pathname;
    ...
    executeOnFiles: function(files) {
        ...
        if (options.ignore) {
            files.forEach(function(file) {
                // Since `file` can be a glob pattern, we need the try-catch
                // around`fs.statSync` calls. We cannot use `fs.existsSync`
                // since it is deprecated.
                try {
                    var fullPath = path.resolve(file);
                    if (fs.statSync(fullPath).isFile() && !processed[fullPath]) {
                        results.push(createIgnoreResult(file));
                    }
                } catch (err) {
                    if (err.code !== "ENOENT") {  // only ignore fs.statSync related errors
                        throw err;
                    }
                }
            });
